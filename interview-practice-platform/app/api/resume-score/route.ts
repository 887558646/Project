...
export async function POST(req: NextRequest) {
  ...
  const prompt = `你是一位嚴格的履歷審查官，請針對下方自傳內容，回傳一個JSON，內容包含：\n1. overallScore: 整體分數（0-100，嚴格標準）\n2. categories: 陣列，每個元素包含 name（面向名稱）、score（0-100）、feedback（具體建議）。面向包括：邏輯結構、動機明確度、個人化程度、語言表達。\n請你只回傳純JSON，前後不要有任何說明、標題、註解或多餘文字，否則會導致解析失敗。\n自傳內容：\n${resume}`
  try {
