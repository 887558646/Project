...
/** @typedef {{category:string, question:string, hint:string}} Q */
/** @type {Record<string, Q[]>} */
const S = {
  "台大": [
  ...
  "銘傳大學": [
    { category: "academic", question: "規劃社群輿情與品牌健康度監測：資料蒐集、模型與儀表板如何設計？", hint: "來源、標註、情緒/主題模型、視覺化、異常警示" },
    { category: "career", question: "企業行銷成效歸因專案，如何處理跨通路與跨裝置的識別？", hint: "ID 合併、Cookie 限制、MMM/多觸點、業務指標" },
    { category: "personal", question: "將設計思維導入資料產品的案例：如何釐清痛點並驗證雛形？", hint: "同理訪談、旅程地圖、線框/原型、可用性測試" },
    { category: "academic", question: "為影像/短影音做推薦，如何處理冷啟動與同質化？", hint: "特徵工程、混合式推薦、探索/利用、評估多樣性" },
    { category: "motivation", question: "在銘傳如何結合傳播與資訊，打造具影響力的資料敘事？", hint: "議題、來源、敘事結構、互動設計、傳播渠道" },
  ],
  "實踐大學": [
    { category: "academic", question: "以使用者研究為核心設計數位服務，資料如何支持設計決策？", hint: "族群、方法、指標、實驗與追蹤、迭代機制" },
    { category: "career", question: "與時尚/家設品牌合作，如何以數據優化商品企劃與補貨？", hint: "銷售資料、供應鏈限制、季節/流行、預測模型、營運KPI" },
    { category: "personal", question: "把分析結果轉化為視覺設計的經驗，如何兼顧美感與可讀性？", hint: "層級、色彩、標註/圖例、可及性、迭代" },
    { category: "academic", question: "產學合作專題的版本管理與交付驗收如何規劃？", hint: "里程碑、需求凍結、變更單、驗收標準、回顧" },
    { category: "motivation", question: "為何適合在實踐？提出一個想落地的生活創新專題。", hint: "痛點、解法、資源、產出、衡量" },
  ],
  "世新大學": [
    { category: "academic", question: "針對假資訊與深偽影像，設計跨來源的偵測系統與查核流程。", hint: "來源比對、影像/文本模型、可信度、人工複核、發布" },
    { category: "career", question: "在媒體/公關產業，如何用數據支持內容選題與投放？", hint: "受眾分群、成效指標、A/B 測試、競品追蹤、聲量" },
...
async function ensureSchool(school, qs) {
  const existing = await prisma.question.findMany({ where: { school }, select: { question: true } });
  const have = new Set(existing.map(e => e.question));
  let created = 0;
  for (const t of qs) {
    if (have.has(t.question)) continue;
    await prisma.question.create({ data: { question: t.question, hint: t.hint, category: t.category, school, isActive: true } });
    created += 1;
  }
  const total = await prisma.question.count({ where: { school } });
  console.log(`${school}: 新增 ${created} 題，現有 ${total} 題`);
}

(async () => {
  for (const [school, qs] of Object.entries(S)) {
    await ensureSchool(school, qs);
  }
